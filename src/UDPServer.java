            ...
            while (this.serverWorks) {
                ...
                if (message.equals("UPDATE-ROUTING-TABLE")){
                    ...
                    for (Map.Entry<Integer, Neighbor> set : r.neighborsProperties.entrySet()) {
                        ...
                        String replace = neighborDistanceVectorStr.replace("[","").replace("]","");
                    ...
                    for(int index=1; index<=this.r.numNetRouters; index++) {
                        if(index != this.r.routerName){
                            ...
                            for (Map.Entry<Integer, Neighbor> entry : r.neighborsProperties.entrySet()){
                                if (minVal > entry.getValue().neighborDistanceVector.get(index) + this.r.routingTable.distance.get(entry.getKey())){
                                    minVal = entry.getValue().neighborDistanceVector.get(index) + this.r.routingTable.distance.get(entry.getKey());
                                    zKey = entry.getKey();
